...
func (api *KademliaRESTAPI) RegisterRoutes() {
	// Make registration idempotent so tests can create multiple API instances
	// without re-registering the same handlers on the DefaultServeMux.
	registerOnce.Do(func() {
		...
		http.HandleFunc("/objects", func(w http.ResponseWriter, r *http.Request) {
			...
			case http.MethodDelete:
				...
				err = api.kademliaNode.StopRefresh(id)
				if err != nil {
					http.Error(w, "Failed to stop refreshing value", http.StatusInternalServerError)
					return
				}
